...
import { decimalsDivider, splitToAddressesArray } from '@app/common/utils';
...
import { AlpacaSubgraph } from '../../../subgraphs/subgraphs/alpaca.subgraph';
import { AlpacaApiService } from './alpaca.api.service';
import { LocalMultiCall } from './alpaca.local.multi.call';
import { alpacaFactoriesMap, alpacaLegacyToken, alpacaRewardToken } from './contracts/alpaca.abi';
import { LeverageErcToken, LeverageFarmingPositionDto } from './dto/alpaca.dto';
import {
...
import { LeverageFarming, LeverageFarmingPosition } from './interfaces/leverage.farming.interfaces';
...
export class AlpacaService {
  ...
  async getDataByAddresses(address: Address, chain: ChainDto): Promise<BaseData[]> {
    ...
    // TODO fix address - was string, did string[]
    // TODO fix
    // localMultiCall.getLendingPoolsBalances(address, setTokenAddresses),
    const [lendingTokens, leverageFarming] = await Promise.all([
      localMultiCall.getLendingPoolsBalances(splitToAddressesArray(address), setTokenAddresses),
    ...
    await Promise.all([
      ...
      localMultiCall.getLpTokenData(leverageFarmingPositions, setTokenAddresses),
    ]);
