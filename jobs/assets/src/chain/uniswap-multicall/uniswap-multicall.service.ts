...
export class UniswapMulticallService {
  ...
  async getTokenDecimals(tokens: string[]): Promise<{ [token: string]: number }> {
    ...
    const { results: { transformed } } = await aggregate(calls, {
      web3: this.web3,
  ...
  async allPairsLength(factoryAddress: string): Promise<number> {
    const { results: { transformed } } = await aggregate({
  ...
  async getPairReserves(pairs: string[]): Promise<{ [pair: string]: { reserve0: BigNumber, reserve1: BigNumber } }> {
    const calls = pairs.map(pair => ({
      target: pair,
