...
@Injectable()
export class PoolsServiceCommon {
  constructor(
    @Inject(UniswapSubgraph) private readonly uniswapSubgraph: UniswapSubgraph,
    @Inject(SushiswapSubgraph) private readonly sushiswapSubgraph: SushiswapSubgraph,
    @Inject(BlocksSubgraph) private readonly blocksSubgraph: BlocksSubgraph,
  ) {
  }
  ...
  // get up to 3k pools with all necessary calculations, processing can be up to 10 sec
  async getSushiswapPoolsToHandle(): Promise<PoolDto[]> {
    const [blockDataDayBefore, blockDataWeekBefore, blockDataMonthBefore] = await Promise.all([
      this.blocksSubgraph.getFirstAfterTimestamp(TIMESTAMP_DAY_BEFORE_CURRENT),
      this.blocksSubgraph.getFirstAfterTimestamp(TIMESTAMP_WEEK_BEFORE_CURRENT),
      this.blocksSubgraph.getFirstAfterTimestamp(TIMESTAMP_MONTH_BEFORE_CURRENT),
    ...
    const [
      ...
      this.sushiswapSubgraph.getPairs(MIN_RESERVE),
      this.sushiswapSubgraph.getPairsInBlockState(MIN_RESERVE, lastDayBlockNumber),
      this.sushiswapSubgraph.getPairsInBlockState(MIN_RESERVE, lastWeekBlockNumber),
      this.sushiswapSubgraph.getPairsInBlockState(MIN_RESERVE, lastMonthBlockNumber),
    ]);
