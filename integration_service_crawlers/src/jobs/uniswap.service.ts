...
export class UniswapService {
  protected integrationName: string = 'uniswap'
  constructor(
  ...
  public async startMigration(): Promise<any> {
    let blockNumbersToMigrate = await this.buildBlockNumberArray()
  ...
  private async buildBlockNumberArray(): Promise<Array<number>> {
    ...
    if (migrationStartBlock == null) {
      ...
      this.logger.log(`starting initial [${this.integrationName}] data migration from block [${migrationStartBlock}]`)
    } else {
      migrationStartBlock++
    }
    ...
    this.logger.log(`starting [${this.integrationName}] data migration from [${migrationStartBlock}] to [${migrationEndBlock}], blocks count: [${migrationEndBlock - migrationStartBlock}]`)
  ...
  protected async handleConcurrently(blockNumbers: Array<number>): Promise<string> {
    ...
    for (let i = 0, j = blockNumbers.length; i < j; i += chunkSize) {
      ...
      try {
        ...
        this.logger.log(`error during processing ${this.integrationName} blocks from [${chunksArray[0]}] to [${chunksArray[chunksArray.length - 1]}]`)
        await this.dbTransactionManager.rollback()
        return blockNumbers.length.toString().concat(' imported with error')
      }
      this.logger.log(`blocks processing done ${this.integrationName} blocks from [${chunksArray[0]}] to [${chunksArray[chunksArray.length - 1]}] count [${chunksArray[chunksArray.length - 1] - chunksArray[0] + 1}]`)
