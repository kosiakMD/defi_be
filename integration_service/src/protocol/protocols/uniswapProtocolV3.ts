...
import { LiquidityPosition } from 'src/dto/liquidity.position.dto';
import { CurrentPricesPayload, PriceResponseDto } from 'src/dto/price.response.dto';
import { LiquidityPoolFeature, PoolTokenDto } from 'src/integrations/integrations.dto';
import { ERC20Token, PoolToken } from 'src/interfaces/transactions.interfaces';
import { PriceService } from 'src/price/price.service';
import { FeatureEnum } from 'src/protocol/features/features.enum';
import AbstractProtocol from 'src/protocol/protocols/abstractProtocol';
import BasicProtocol from 'src/protocol/protocols/basicProtocol';
import { UniswapV3Subgraph } from 'src/thegraph/uniswap.v3.subgraph';
import { UniswapV3Position } from 'src/thegraph/uniswap/position.dto';
import { ClaimAbleTokenDto } from 'src/uniswap/dto/claimable.token.dto';
import { objectUpdate } from 'src/utils/object';
import { calculatePositionAmounts, calculateTokensOwed } from 'src/utils/uniswapV3PositionMath';

import { Inject, Injectable } from '@nestjs/common';
import { WINSTON_MODULE_NEST_PROVIDER } from 'nest-winston';

import { ChainAbbrEnum, ChainIdEnum, ProjectEnum, UniswapProtocolEnum } from 'src/common/enum';
...
export class UniswapProtocolV3 extends BasicProtocol implements AbstractProtocol {
  ...
  constructor(
    ...
    private readonly uniswapV3Subgraph: UniswapV3Subgraph,
  ...
  mapToLiquidityPosition(
    ...
    return positions
      .map((position) => {
        ...
        const lpToken = plainToClass(ERC20Token, {
          address: position.pool.id,
          decimals: 0,
          totalSupply: 1,
        });

        const token0 = plainToClass(PoolTokenDto, {
        ...
        const token1 = plainToClass(PoolTokenDto, {
        ...
        return plainToClass(LiquidityPosition, {
  ...
  protected async transformPools(
    ...
    chainId: ChainIdEnum,
  ): Promise<{ errors: any[]; data: FeatureResultDto<LiquidityPoolFeature> }> {
    const result = {
      errors: [] as any[],
      data: {
        totalValue: 0,
        items: [],
      } as FeatureResultDto<LiquidityPoolFeature>,
    };
    ...
    const outputPools: LiquidityPoolFeature[] = rawPools?.reduce((resultArray, inputPool) => {
