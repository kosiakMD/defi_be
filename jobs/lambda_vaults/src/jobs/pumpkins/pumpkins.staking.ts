...
import { PumpkinsAddresses } from './addresses';

@Injectable()
export class PumpkinsStaking implements JobInterface {
  ...
  async buildInitialMapping(jobMapping: TrackedVault): Promise<TrackedVault> {
    ...
    > = await this.getAllPoolInfo(PumpkinsAddresses.chief);
    ...
    for (const address of poolsInfo.keys()) {
      try {
        ...
        const stakingPoolFeature: IntegrationStakingPositionDto = plainToClass(
          IntegrationStakingPositionDto,
          {
            address: PumpkinsAddresses.chief,
  ...
  private async getAllPoolInfo(chiefContract: PumpkinsAddresses): Promise<Map<string, any>> {
  ...
  private async toDbMapping(stakingPosition: IntegrationStakingPositionDto) {
    ...
    const rewardTokenUniqueId = concatStrings(this.chain, PumpkinsAddresses.kin);
  ...
  async updateWithChainData(): Promise<any[]> {
    ...
    const kinsPerBlock: BigNumber = multicallRsp.get(this.kinsPerBlockLabel()).output.data;
    ...
    this.mapping = this.mapping.map((m) => {
      if (m instanceof IntegrationStakingPositionDto) {
        ...
        const aprStats = {
          ...
          rewardTokenPerBlock: toDecimals(kinsPerBlock, m.rewards[0].decimals),
  ...
  private getCallsForChief() {
    return new Map<string, CallData>([
      ...
      [
        this.kinsPerBlockLabel(),
        {
          address: PumpkinsAddresses.chief,
          abi: Abis.kinsPerBlock,
          input: {
            data: [],
          },
          output: {},
  ...
  private balanceOfLabel(stakingPosition: IntegrationStakingPositionDto) {
    return concatStrings(
      Abis.balanceOf.name,
      PumpkinsAddresses.chief,
      stakingPosition.stakingToken.address,
    );
  }

  private poolInfoLabel(stakingPosition: IntegrationStakingPositionDto) {
    return concatStrings(Abis.poolInfo.name, PumpkinsAddresses.chief, stakingPosition.poolId);
  }

  private totalAllocPointLabel() {
    return concatStrings(Abis.totalAllocPoint.name, PumpkinsAddresses.chief);
  }

  private kinsPerBlockLabel() {
    return concatStrings(Abis.kinsPerBlock.name, PumpkinsAddresses.chief);
  }

  private poolLengthLabel() {
    return concatStrings(Abis.poolLength.name, PumpkinsAddresses.chief);
  }
