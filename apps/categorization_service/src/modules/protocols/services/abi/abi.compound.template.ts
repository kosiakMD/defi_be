...
/**
 * Beefy.Finance: BIFI Maxi Vault
 * Info taken from: https://bscscan.com/address/0xf7069e41c57ecc5f122093811d8c75bdb5f7c14e#readContract
 */
export const AbiCompoundTemplate = {
  id: -2,
  address: '0xf7069e41C57EcC5F122093811d8c75bdB5f7c14e',
  functionPredicates: [
    { predicate: (item) => contains(item.name, 'totalSupply') },
    { predicate: (item) => contains(item.name, 'strategy') },
    { predicate: (item) => contains(item.name, 'want') },
    { predicate: (item) => contains(item.name, 'balance') },
    { predicate: (item) => contains(item.name, 'getPricePerFullShare') },
    { predicate: (item) => contains(item.name, 'decimals') },
  ],
