...
export class BalancerFirstCheckJob {
	...
	public async crawl_new_tokens(job: any, done: any): Promise<void> {
		try {
			...
			for (let i = 0; i < tokens.length; i++) {
				...
				console.log(
					'checking token-pool  ' + tokens[i]['id'] + ' with token length ' + pool_tokens.length,
				);
				if (parseInt(tokens[i]['totalShares']) !== 0) {
					...
					for (let z = 0; z < pool_tokens.length; z++) {
						try {
							...
							const current_db_token = await this.databaseService.getTokenByAddress(
								pool_tokens[z]['id'].split('-')[1],
							);
							if (current_db_token.length) {
								const current_db_token_price = await this.databaseService.getTokenPrice(
									current_db_token[0]['id'],
								);
	...
	public crawl_new_tokens_history = async (job: any, done: any): Promise<void> => {
		...
		const db_assets = await this.databaseService.getNewTokensByResource('BALANCER');
		console.log('starting uniswap history clawler');

		const first_tx_data = await this.theGraphService.getBalancerfirstTxTimestamp();
		...
		for (let i = 0; i < db_assets.length; i++) {
			...
			let prices_count = 0;
			do {
				const first_day_block_query = await this.theGraphService.getBalancerfirstBlockQuery(
					check_day_ts,
				);
				const block_number = first_day_block_query['data']['data']['blocks'][0]['block'];
				console.log('block_number ', block_number);

				const daily_price_query = await this.theGraphService.getBalancerDailyBlockPricesQuery(
					parseInt(block_number),
					db_assets[i]['address'],
				);
				console.log(daily_price_query['data']['data']);
				if (daily_price_query['data']['data']['pools'].length) {
					const { totalShares, tokens } = daily_price_query['data']['data']['pools'][0];

					const pool_tokens = tokens;
					let lp_token_price = 0;
