...
export class PoolsServiceBalancer {
	...
	constructor(@Inject(BalancerSubgraph) protected readonly balancerSubgraph: BalancerSubgraph,
	...
	private getPairsWithCalculatedFields(pools: Pool[], [lastDayPools, lastWeekPools, lastMonthPools]): LiquidityPool[] {
		return pools.map(pool => {
			...
			const dayAPY: number = lastDayPool ? getLastDayApy(pool.liquidity, pool.totalSwapVolume - lastDayPool.totalSwapVolume) : null
			const weekAPY: number = lastWeekPool ? getLastWeekApy(pool.liquidity, pool.totalSwapVolume - lastWeekPool.totalSwapVolume) : null
			const monthAPY: number = lastMonthPool ? getLastMonthApy(pool.liquidity, pool.totalSwapVolume - lastMonthPool.totalSwapVolume) : null

			const currentLpToken: number = getLpTokenPrice(pool.liquidity, pool.totalShares)
			const dayIL: number = lastDayPool ? getImpermanentLossPercent(currentLpToken, getLpTokenPrice(lastDayPool.liquidity, lastDayPool.totalShares)) : null
			const weekIL: number = lastWeekPool ? getImpermanentLossPercent(currentLpToken, getLpTokenPrice(lastWeekPool.liquidity, lastWeekPool.totalShares)) : null
			const monthIL: number = lastMonthPool ? getImpermanentLossPercent(currentLpToken, getLpTokenPrice(lastMonthPool.liquidity, lastMonthPool.totalShares)) : null

			const totalWeight: number = pool.tokens.reduce((acc, token) => acc += token.denormWeight, 0)
