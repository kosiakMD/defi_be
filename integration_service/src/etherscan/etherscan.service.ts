...
export class EtherscanService {
  ...
  async getBalances(addresses: string[]): Promise<any> {
    let transfersAll: Transfers = {};
    await Promise.all(addresses.map(async a => {
      const rsp = await this.etherscanApi.getTransfers(a);
      transfersAll[a] = rsp.data.result;
    }));
    const allBalances: BalancesResponse = {};
    Object.keys(transfersAll)
      .forEach(address => {
        ...
        transfersAll[address].map(transfer => {
          ...
          const existedAccountBalance: AccountTokenBalance = allBalances[address].tokens.find(tok => tok.token.token.address === transfer.contractAddress);
          if (existedAccountBalance === undefined) {
            ...
            existedAccountBalance.amount = (Number(existedAccountBalance.amount) + Number(amountToAdd)).toString();
            existedAccountBalance.decimalsAmount = existedAccountBalance.decimalsAmount + decimalAmountToAdd;
            existedAccountBalance.token.amount = Number(existedAccountBalance.token.amount) + Number(amountToAdd);
            existedAccountBalance.token.decimalsAmount = existedAccountBalance.token.decimalsAmount + decimalAmountToAdd;
          }
        });
      });
    return allBalances;
