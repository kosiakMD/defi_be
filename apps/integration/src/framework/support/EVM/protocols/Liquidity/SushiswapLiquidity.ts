...
export type ISushiPoolMeta = IProtocolMeta & {
  subgraphUrl: string;
  minUSDPairsReserve: number;
  pairsPerQuery: number;
  balanceCallsPerQuery: number;
};

export const POOLS_LIST = gql`
  query ($first: Int!, $skip: Int!, $minReserve: BigDecimal!) {
    pairs(first: $first, skip: $skip, where: { reserveUSD_gt: $minReserve }) {
      id
      token0 {
        id
      }
...
export class SushiswapLiquidity
  extends EVMCore<IPoolFeatureMinimal, IPoolFeatureOpportunity, IPoolFeatureUser, ISushiPoolMeta>
  implements IRootProtocol
{
  ...
  async getCacheableOpportunityData(): Promise<IPoolFeatureMinimal[]> {
    const resultPairs = await this.loadPairsData(
      POOLS_LIST,
      this.meta.pairsPerQuery,
      {
        minReserve: this.meta.minUSDPairsReserve,
      },
      'pairs',
    );
    return resultPairs.map((pool) => this.toFeatureEntryMinimal(pool));
  }

  private async loadPairsData(rawQuery, numberPerQuery, variables, path) {
    let collectedData = [];
  ...
  private toFeatureEntryMinimal(pool): IPoolFeatureMinimal {
    return {
      ...
      supplied: [{ token: { address: pool.token0.id } }, { token: { address: pool.token1.id } }],
    };
  }
