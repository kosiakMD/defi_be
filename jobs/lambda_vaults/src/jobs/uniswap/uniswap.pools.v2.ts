...
import { UniswapV2TrackedPairs } from './addresses';

@Injectable()
export class UniswapPoolsV2 implements JobInterface {
  chain = ChainIdEnum.eth;
  feature = FeatureEnum.pools;
  protocol = ProtocolNameEnum.uniswapV2;
  ...
  async updateMapping(jobMapping: TrackedVault): Promise<any> {
    ...
    const pairsLength = UniswapV2TrackedPairs.length;
    for (let i = 0; i < pairsLength; i++) {
      const token = await this.accountService.saveTrackingAsset(
        UniswapV2TrackedPairs[i],
        this.chain,
      );
