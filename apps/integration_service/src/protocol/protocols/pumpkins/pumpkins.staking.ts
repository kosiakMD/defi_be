...
export class PumpkinsStaking {
  ...
  public async getData(addresses: Address[], chain: ChainDto): Promise<BaseDataStaking[]> {
    const key = `${chain.id}_${PumpkinsProtocolEnum.pumpkins}_${FeatureEnum.staking}`;
  ...
  private async getDataWithMulticall(addresses: Address[], pools: NotifyStaking) {
    ...
    for (const address of addresses) {
      for (const pool of pools.items) {
        calls.set(this.pendingTokensLabel(address, pool.address, pool.poolId), {
    ...
    claimableRewards.forEach((d, i) => {
      ...
      claimableRewards[i].pendingkins = new BigNumber(claimableReward.pendingkins.toString());
    });
  ...
  private getStakingPositionsForAddress({ balances, claimableRewards }, pools: NotifyStaking) {
    ...
    balances.forEach((b) => {
      ...
      if (claimableReward) {
        ...
        stakingPosition.rewards[0].claimableData.balance = claimableReward.pendingkins
