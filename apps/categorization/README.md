...
#### Start Services
```bash
docker-compose -f apps/categorization/docker-compose.yml up -d
```

#### Stop Services
Add ```--volume``` option if you want to clean up the data.
```bash
docker-compose -f apps/categorization/docker-compose.yml down
```
...
##### Local Development
```bash
npm run categorization-migration:run-dev
```
##### Environment
```bash
npm run categorization-migration:run
```

#### Migration Down
##### Local Development
```bash
npm run categorization-migration:revert-dev
```
##### Environment
```bash
npm run categorization-migration:revert
```

#### Create New Migration
##### Local Development
```bash
npm run categorization-migration:generate-dev -- -n ${YOUR_MIGRATION_NAME}
```
...
```bash
# watch mode
npm run categorization-start:dev
```
...
#### Technical Details
Categorization Service uses Bull Queue based on Redis to process tasks. These are types of tasks which are currently supported:
