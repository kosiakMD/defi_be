...
export class UniswapCurrentPricesJob {
  ...
  public async crawl(job: any, done: any): Promise<void> {
    console.log("Current UNISWAP Prices Job Sarted")
    try{
      ...
      if (db_assets.length) {
        ...
        for(let i = 0 ; i < db_assets.length; i++){
          ...
          let one_results =  await this.theGraphService.getCurrentUniTokenPrices(db_assets[i]['address'])
    ...
    console.log("Add Current UNISWAP Prices Job done")
    done();
  }
}
