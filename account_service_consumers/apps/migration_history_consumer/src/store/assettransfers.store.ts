...
export class AssetsTransfersStore {
  ...
  async getEventsBetweenBlocks(assetAddress: string, chainId: number, fromBlock: number, toBlock: number): Promise<EventDto[]> {
    ...
    if (chainId === 1) {
      contractEventsQuery = `
        ...
        where eth_events.block_number >= ${fromBlock}
            and eth_events.block_number <= ${toBlock}  
            and address = '${assetAddress}'
    `;
    } else if (chainId === 2) {
      contractEventsQuery = `
        select
        ...
        where bsc_events.block_number between ${fromBlock} and ${toBlock}  and address = (
            select assets.address from assets where assets.id = ${assetAddress}
        )
    `;
    } else {
  ...
  async insert(transfer: AssetTransfersEntity): Promise<any> {
    ...
    const insertQuery = `insert into asset_transfers (asset_id, "from", "to", value, timestamp, tx_hash) 
        values (
                ${transfer.assetId}, 
  ...
  async insertAll(transfers: AssetTransfersEntity[]): Promise<any> {
    ...
    const queryStart = `insert into asset_transfers (asset_id, "from", "to", value, timestamp, tx_hash)
                        values`;
