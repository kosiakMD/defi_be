...
export class GaugeRewards {
	...
	constructor(private readonly httpService: HttpService,
		...
		this.multicallContract = new this.web3.eth.Contract(multicall.abi as AbiItem[], multicall.address)
	}
	async getGaugesRewards(): Promise<any>  {
		...
		const ratesCalls = decodedGauges.map(gauge => [
			[gauge, "0x180692d0"], [gauge, "0x17e28089"],
		])
		const aggRates = await this.multicallContract.methods.aggregate(ratesCalls.reduce((acc, val) => acc.concat(val), [])).call()
		const decodedRate = aggRates[1].map(
			hex => this.web3.eth.abi.decodeParameter('uint256', hex)
		)
		...
		const workingSupplies = decodedRate
			.filter((_, i) => i % 2 == 1)
			.map(v => v / 1e18)
		const virtualPriceCalls = Object.keys(poolInfo).map(key => poolInfo[key])
			.map(v => [
				v.swap,
				"0xbb7b8b80"
			])
		const aggVirtualPrices = await this.multicallContract.methods.aggregate(virtualPriceCalls).call()
		...
		decodedWeights.map((weight, i) => {
			let result = {}
			const pool = Object.keys(poolInfo).map(key => poolInfo[key]).find(value =>
				value.gauge.toLowerCase() == '0x' + weightCalls[i][1].slice(34).toLowerCase()
			).name
			result['swap_address'] = poolInfo[pool].swap
			result['virtual_price'] = decodedVirtualPrices.find(price =>
				price[0].toLowerCase() == result['swap_address'].toLowerCase()
			)[1]
