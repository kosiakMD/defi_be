...
import { DepositEvent, WithdrawalEvent } from '../../../common/interfaces';
import {
  ERC20Transfer,
  ScanTransfer,
  TransfersResponse,
...
@Injectable()
export class WETHContract implements OnModuleInit {
  ...
  static depositsToTransfersResponse(deposits: DepositEvent[]): TransfersResponse<ScanTransfer> {
    ...
    deposits.map((deposit) => {
      ...
      const erc20Transfer: ERC20Transfer = {
        fromAddress: WETHContract.token.address,
        toAddress: userAddress,
        amount: deposit.returnValues.wad,
        tokenPriceUSD: null,
        totalPriceUSD: null,
        token: {
          address: WETHContract.token.address,
          chainId: WETHContract.token.chainId,
          name: WETHContract.token.name,
          symbol: WETHContract.token.symbol,
          decimals: WETHContract.token.decimals,
        },
      };
  ...
  static withdrawalsToTransfersResponse(
    ...
    withdrawals.map((withdrawal) => {
      ...
      const erc20Transfer: ERC20Transfer = {
        ...
        toAddress: WETHContract.token.address,
