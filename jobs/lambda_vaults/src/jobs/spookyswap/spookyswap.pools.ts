...
import { SpookySwapAddresses } from './addresses';

@Injectable()
export class SpookyswapPools implements JobInterface {
  chain = ChainIdEnum.ftm;
  feature = FeatureEnum.pools;
  protocol = ProtocolNameEnum.SpookySwap;
  ...
  async buildInitialMapping(jobMapping: TrackedVault): Promise<any> {
    ...
    for (let i = poolIdFrom; i <= poolIdTo; i++) {
      calls.set(this.poolInfoLabel(i), {
        address: SpookySwapAddresses.chief,
        abi: Abis.poolInfo,
        input: {
          data: [i],
        },
  ...
  poolLengthLabel() {
    return concatStrings(Abis.poolLength.name, SpookySwapAddresses.chief);
  }

  poolInfoLabel(poolId) {
    return concatStrings(Abis.poolInfo.name, SpookySwapAddresses.chief, poolId);
  }
