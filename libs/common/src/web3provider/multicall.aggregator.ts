...
export class MulticallAggregator {
  ...
  async handleInBatches<T = any>(
    ...
    const { returnData } = requireSuccess
      ? await multicall.tryAggregate(callsToBeExecuted)
      : await multicall.aggregate(callsToBeExecuted);
  ...
  async callArray(calls: CallData[], chain: ChainIdEnum, requireSuccess = true): Promise<any[]> {
    const callMap = new Map<string, CallData>();
    calls.forEach((call) => {
      callMap.set(this.key(call), call);
    });
    const resultsMap = await this.handleInBatches(callMap, chain, requireSuccess);
    const results = [];
    calls.forEach((call) => {
