...
export class VaultsServiceSushiswap {
	...
	async getVauts(): Promise<Vault[]> {
		...
		const liquidityPools = await this.sushiswapSubgraph.getVaultsData(vaultsData.data.pools.map(p => p.pair))
		const masterChief = vaultsData.data.chief[0]
		const avgBlockTime = (blocks.data.blocks[0].timestamp - blocks.data.blocks[blocks.data.blocks.length - 1].timestamp) / 1000
		...
		return vaultsData.data.pools.map(vault => {
			...
			if (liquidityPool != undefined) {
				...
				const vaultTVL = (vault.balance * (10 ** (-18))) / Number(liquidityPool.totalSupply) * Number(liquidityPool.reserveUSD)
	...
	public calculateAPY(apyData: SushiAPYData): number {
		const poolRewardPerBlock = apyData.allocPoints / apyData.totalAllocPoints * apyData.rewardTokenPerBlock * apyData.rewardTokenPrice
