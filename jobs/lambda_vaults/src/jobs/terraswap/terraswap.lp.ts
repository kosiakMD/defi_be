...
@Injectable()
export class TerraSwapLp extends JobPoolsBase<LiquidityPoolFeature> {
  ...
  async rebuildMapping(jobMapping: TrackedVault): Promise<any> {
    ...
    const terraswapPools: FactoryPairsInfoResp[] = await this.getTerraSwapPools();
    const poolIdTo = terraswapPools.length;
  ...
  async getTerraSwapPools() {
    const terra = this.web3ProviderService.getInstanceByChainId(this.chain);
    const { pairs } = await terra.wasm.contractQuery(TerraSwapAddresses.tokenFactory, {
      pairs: { limit: 100 },
    });
    return pairs;
  }
