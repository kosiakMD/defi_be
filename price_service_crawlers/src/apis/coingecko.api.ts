...
export const getCurrentCoinPrices = (addresses) => http.get(`${baseUrl}/simple/token_price/ethereum?contract_addresses=${addresses}&vs_currencies=${CURRENCY}`);

export const getCoinRangePrices = (http_client, token, from, to) =>
  http_client.get(`${baseUrl}/coins/ethereum/contract/${token}/market_chart/range?vs_currency=`+CURRENCY+`&from=${from}&to=${to}`);