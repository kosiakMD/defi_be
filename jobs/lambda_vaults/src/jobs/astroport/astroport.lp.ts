...
@Injectable()
export class AstroportLp extends JobPoolsBase<LiquidityPoolFeature> {
  ...
  async rebuildMapping(jobMapping: TrackedVault): Promise<any> {
    ...
    const astroportPools: FactoryPairsInfoResp[] = await this.getAstroportPools();
    const poolIdTo = astroportPools.length;
