...
import TraderJoeProtocol from './trader-joe.protocol';

@Injectable()
export class TraderJoePools {
  constructor(
  ...
  public async getData(addresses: Address[], chain: ChainDto): Promise<BaseData[]> {
    const cacheKey = `${chain.id}_${TraderjoeProtocolEnum.traderjoe}_${FeatureEnum.pools}`;
  ...
  private toPosition(balance: TokenBalance, poolData: LiquidityPoolFeature): LiquidityPoolFeature {
    ...
    userData.stats.feeRate = TraderJoeProtocol.feeRate;
