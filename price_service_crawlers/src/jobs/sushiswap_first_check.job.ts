...
export class SushiSwapFirstCheckJob {
  ...
  public async crawl_new_tokens(job: any, done: any): Promise<void> {
    ...
    try{
      ...
      let tokenRequest = await this.theGraphService.getSushiswapPoolsTokens();
  ...
  public crawl_new_tokens_history = async (job: any, done: any): Promise<void> => {
      ...
      let db_assets =await this.databaseService.getNewTokensByResource('SUSHISWAP');
      console.log('starting')

      let first_tx_data= await this.theGraphService.getSushiswapfirstTxTimestamp();
      ...
      do {
        ...
        let first_day_block_query  =  await this.theGraphService.getSushiswapfirstBlockQuery(check_day_ts);
        const block_number = first_day_block_query['data']['data']['blocks'][0]['blockNumber'];
        console.log("block_number ",block_number)

        let daily_price_query = await this.theGraphService.getSushiswapDailyBlockPricesQuery(parseInt(block_number), db_assets[i]['address']);
...
async function crawlCoin(coin_id, coin, prices, currency_id, db) {
  ...
  console.log(`sushi ${prices.length} prices found`);
  if(prices.length){
