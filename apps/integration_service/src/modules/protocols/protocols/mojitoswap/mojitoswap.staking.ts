...
export class MojitoswapStaking {
  ...
  private async getAutostakingVaultBalance(addresses: string[], chain: ChainDto) {
    ...
    const vaultRsp: Map<string, ICallData> = await this.multicallService.handleInBatches(vaultCalls, chain.id);
    ...
    addresses.forEach((userAddress) => {
      ...
      if (userShares > 0) {
        const userBalance = new BigNumber(userShares).div(totalShares) //
