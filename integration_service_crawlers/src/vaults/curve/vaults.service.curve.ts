...
export class VaultsServiceCurve {
	...
	async getVauts(): Promise<Vault[]> {
		...
		const stakingPoolPositions = await this.curveSubgraph.getLiquidityPositions(stakingPoolsAddresses)
		const ratios = {}
		...
		return this.getFormatedVaults(curvePools.data.pools, curveApys, CRVToken, ratios, [...coinPrices, ...eurPrice])
	}

	private getFormatedVaults(pools, rewards, rewardToken, ratios, prices): Vault[] {
		return pools.filter(pool => typeof this.getAPYYear(pool, rewards) === 'number').map(pool => {
