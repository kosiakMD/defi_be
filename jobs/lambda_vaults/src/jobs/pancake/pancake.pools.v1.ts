...
import { PancakeAddresses, PancakeV1TrackedPairs } from './addresses';

@Injectable()
export class PancakePoolsV1 implements JobInterface {
  chain = ChainIdEnum.bsc;
  feature = FeatureEnum.pools;
  protocol = ProtocolNameEnum.pancakeV1;
  ...
  async updateMapping(jobMapping: TrackedVault): Promise<any> {
    ...
    const promises = [];
    PancakeV1TrackedPairs.forEach((address) => {
      promises.push(this.accountService.saveTrackingAsset(address, this.chain));
    });
